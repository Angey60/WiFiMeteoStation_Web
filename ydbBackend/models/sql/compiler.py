...
def _generate_params_for_update(placeholder_rows, columns, field_types, params):
    ...
    modified_params = {}
    #logger.info(f'placeholder_rows: {placeholder_rows}')
    
    for i in range(len(placeholder_rows)):
        #if model_types != []: # !!!
        model_type = model_types[i]
        placeholder_row = placeholder_rows[i]
        #logger.info(f'model_types[{i}]: {model_type} placeholder_row: {placeholder_row}')
        if str(model_type) == "DateTimeField": 
            modified_params[placeholder_row] = (
                int(params[i].timestamp()),
                _ydb_types[model_type]
            )
        else:
            modified_params[placeholder_row] = (
                params[i],
    ...
    #logger.info(f'modified_params: {modified_params}')
