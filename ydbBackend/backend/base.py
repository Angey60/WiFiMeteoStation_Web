...
class DatabaseWrapper(BaseDatabaseWrapper):
    ...
    def get_database_version(self):
        ...
        try:
            ...
            for item in all_table_names:
                logger.info(f'-> {item}')
            #=========================================================
            '''
            with self.connection.cursor() as cursor:
                cursor.execute("SELECT note FROM devices")
                row = cursor.fetchone()
                if row:
                    logger.info(row)
    ...
    def check_database_version_supported(self):
        ...
        if (
                ...
                #and int(self.get_database_version()) < self.features.minimum_database_version
        ):
    ...
    def get_connection_params(self):
        ...
        if not settings_dict.get("NAME"):
            raise ImproperlyConfigured(
                "YDB database is not configured. Set 'NAME' in DATABASES."
            )
        
        if not settings_dict.get("PROTOCOL"):
            raise ImproperlyConfigured(
                "YDB host is not configured. Set 'PROTOCOL' in DATABASES."
            )
        ...
        #logger.info(f"conn_params: {conn_params}")
    ...
    def get_new_connection(self, conn_params):
        ...
        """
        #logger.info(f"Connecting to YDB with params: {conn_params}")
